# 04. Web Top 10 (2021) Checklist

**Doctrine ID:** `04-web-top10-checklist`
**Mode:** review
**Source standards:** OWASP Top 10 (2021), categories A01 to A10

---

## Purpose

This doctrine runs the ten 2021 Top 10 categories over `<target-repo>` as a
...
3. **A category is not a finding.** "This app has A03" is not something you can
...
uneven: A03 is a sink shape you can grep for, A04 is an entire discipline.
...
features one at a time and adding authorization per handler by hand.
...
that only call it in one branch. Deeper doctrine: `doctrine/13-auth-and-session.md`.
...
protection and is not. The 2021 rename away from "sensitive data exposure" was
deliberate: the failure is usually a bad choice of primitive or mode rather
than an absence of crypto. Home-rolled encryption, ECB, a static IV, a password
hashed with a fast general-purpose digest, or TLS terminated somewhere and then
carried onward in plaintext.
...
| # | Control | ASVS ref | Evidence | Verdict |
|---|---|---|---|---|
| 04.2.1 | Passwords are stored with a memory-hard function (Argon2id, scrypt, bcrypt) with parameters read and confirmed | V2.4 | | |
| 04.2.2 | No home-rolled cipher construction; authenticated encryption is used where confidentiality is needed | V6.2 | | |
| 04.2.3 | IV or nonce is unique per operation and not a constant read from configuration | V6.2 | | |
| 04.2.4 | Security-relevant random values use a cryptographic generator, not the language default | V6.3 | | |
| 04.2.5 | TLS is enforced on every hop, including internal service calls and callbacks | V9.1 | | |
| 04.2.6 | Keys are separated from the data they protect and are rotatable | V6.4 | | |
| 04.2.7 | Sensitive fields are not cached, logged, or included in error responses | V8.1 | | |
| 04.2.8 | Deprecated primitives (MD5, SHA-1, DES, RC4) are absent from security paths | V6.2 | | |
...
objects. Cross-site scripting moved into this category in 2021 because it is
the same defect against a different interpreter, the browser. The fix is never
escaping alone; it is a parser that keeps code and data in separate channels.
...
| # | Control | ASVS ref | Evidence | Verdict |
|---|---|---|---|---|
| 04.3.1 | All queries use parameter binding; no user data reaches a query string by concatenation | V5.3 | | |
| 04.3.2 | Identifiers that cannot be bound (table, column, sort direction) are allowlisted against a fixed set | V5.3 | | |
| 04.3.3 | Every self-written query helper has been opened and read, not trusted by name | V5.3 | | |
| 04.3.4 | OS command execution passes arguments as a vector, never a shell string | V5.3 | | |
| 04.3.5 | Output encoding is contextual and applied at the sink, not at the input boundary | V5.3 | | |
| 04.3.6 | Auto-escaping is not disabled in templates, or every exception is justified and reviewed | V5.3 | | |
| 04.3.7 | User input does not reach a template compiler, expression evaluator, or deserializer | V5.5 | | |
| 04.3.8 | Input validation exists as defence in depth but is not the primary control against injection | V5.1 | | |
...
component is compromised. This category was added in 2021 to say plainly that
some things cannot be fixed by writing the same feature more carefully.
...
no search pattern. A pass that reports nine categories examined and A04 silently
...
| # | Control | ASVS ref | Evidence | Verdict |
|---|---|---|---|---|
| 04.4.1 | A threat model exists for this review (step 2) and names assets, adversaries, and trust boundaries | V1.1 | | |
| 04.4.2 | Security-relevant flows have documented limits: attempts, amounts, rates, quantities | V1.1 | | |
| 04.4.3 | Multi-step flows enforce step order and cannot be resumed from an arbitrary step | V1.1 | | |
| 04.4.4 | Failure modes are closed: when a dependency is unavailable, the flow denies rather than proceeds | V1.1 | | |
| 04.4.5 | Trust boundaries are explicit and each one has a stated control, not an assumption | V1.1 | | |
| 04.4.6 | Recovery and support flows (reset, impersonation, override) are modelled as attack surface | V1.2 | | |
| 04.4.7 | Segregation of tenants or user data is a design property, not a per-query habit | V1.1 | | |
...
features enabled that nothing uses, security headers absent. It is the category
most likely to be invisible in a pure code read, because the truth lives in
deployment state rather than in the tree.
...
| # | Control | ASVS ref | Evidence | Verdict |
|---|---|---|---|---|
| 04.5.1 | Debug and verbose modes cannot be enabled in production by a request-controlled value | V14.1 | | |
| 04.5.2 | Error responses to clients carry no stack trace, framework version, or query text | V7.4 | | |
| 04.5.3 | Configuration defaults to the secure value when a setting is missing | V14.1 | | |
| 04.5.4 | No default or example credentials remain in any configuration path | V14.1 | | |
| 04.5.5 | Security headers are set and their values were read, not assumed from a library name | V14.4 | | |
| 04.5.6 | Unused framework features, sample content, and admin consoles are not deployed | V14.2 | | |
| 04.5.7 | Directory listing and source disclosure are disabled on every served path | V14.3 | | |
| 04.5.8 | Cloud storage and object store permissions are declared in code and are not public by default | V14.1 | | |

---

### A06: Vulnerable and Outdated Components

You are running a version of something with a known, published defect. This
category is unusual: the vulnerability is not yours and the fix is not a code
change, so it gets deferred indefinitely. It matters because exploitation is
cheap once the advisory is public, and because the dependency runs with your
application's full privilege regardless of how carefully your own code is
written.
...
depth, because the risk is rarely in what you chose directly. Find the pinning
strategy and confirm whether an install today would produce the same tree.
Deeper doctrine: `doctrine/09-dependency-supply-chain.md`.

| # | Control | ASVS ref | Evidence | Verdict |
|---|---|---|---|---|
| 04.6.1 | A lockfile exists for every ecosystem and is committed | V14.2 | | |
| 04.6.2 | The full dependency tree, including transitives, is enumerable | V14.2 | | |
| 04.6.3 | Known-vulnerable versions are identified against the resolved tree, not the declared ranges | V14.2 | | |
| 04.6.4 | Components no longer maintained upstream are identified as such | V14.2 | | |
| 04.6.5 | Client-side libraries served to browsers are inventoried alongside server dependencies | V14.2 | | |
| 04.6.6 | Base images and runtime versions are pinned and current | V14.2 | | |
...
be. The failures cluster around the edges rather than the login form: session
...
mechanism with no signature check, a build pipeline that pulls a mutable tag, and
a third-party script included from a host you do not control.
...
is fetched at build or run time and read whether the artifact is verified. Read
the pipeline definition as an attack surface: what can it fetch, what secrets
does it hold, and what would a compromised step be able to do. Deeper doctrine:
`doctrine/09-dependency-supply-chain.md`.

| # | Control | ASVS ref | Evidence | Verdict |
|---|---|---|---|---|
| 04.8.1 | Untrusted input is never deserialized into arbitrary object types | V5.5 | | |
| 04.8.2 | Dependencies resolve from verified sources with integrity hashes recorded | V14.2 | | |
| 04.8.3 | Build pipeline steps are pinned to immutable references, not mutable tags | V14.2 | | |
| 04.8.4 | Externally hosted scripts carry integrity attributes or are self-hosted | V14.2 | | |
| 04.8.5 | Client-supplied state that the server later trusts is signed or held server side | V5.5 | | |
| 04.8.6 | Auto-update or plugin loading paths verify a signature before execution | V14.2 | | |
...
and only partially. This is the category with no exploit and no proof of
concept, which is why it is consistently under-reported: nothing visibly breaks.
It matters because every other category's worst case is much worse when the
detection window is measured in months, and because an audit trail you cannot
reconstruct is an incident you cannot scope.
...
| 04.9.8 | Audit tables that exist in the schema actually have write calls on the paths they cover | V7.2 | | |

---

### A10: Server-Side Request Forgery

The application fetches a URL that an attacker influenced, and the fetch runs
from inside your network with your credentials attached. New to the Top 10 in
2021 by community survey rather than data, which tells you it is under-reported
rather than uncommon. Its severity comes from position: the request originates
somewhere an outsider cannot reach, so it turns a URL field into a probe of your
internal surface and often into a read of instance metadata.

**Search strategy (by idiom):** find every outbound HTTP client construction and
trace the URL argument backwards. The dangerous shape is any request field that
becomes a host, a path, or a whole URL: webhook registration, avatar or document
import by link, PDF and thumbnail rendering, link preview, XML parsing with
external entities enabled, and any callback address supplied by the client. Read
whatever validation exists carefully, because deny-lists and pre-fetch checks
that do not survive a redirect are the standard broken control here.

| # | Control | ASVS ref | Evidence | Verdict |
|---|---|---|---|---|
| 04.10.1 | Every outbound fetch whose URL is client-influenced is enumerated | V12.6 | | |
| 04.10.2 | Destinations are allowlisted by host, not filtered by a deny-list | V12.6 | | |
| 04.10.3 | Redirects are not followed, or the destination is re-validated after each hop | V12.6 | | |
| 04.10.4 | Requests to loopback, link-local, and private ranges are blocked after DNS resolution | V12.6 | | |
| 04.10.5 | Fetch responses are not returned verbatim to the caller | V12.6 | | |
| 04.10.6 | The fetching component holds no ambient credentials the destination could capture | V12.6 | | |
| 04.10.7 | XML and document parsers have external entity resolution disabled | V5.5 | | |
...
the finding. Rule 2 exists because these are the eight ways a Top 10 pass
...
Three category-specific notes:

- **A04** findings frequently fail the verification ceiling honestly. A design
  flaw you inferred from the threat model but could not trace to a concrete
  reachable path is Info and unverified. Say what would lift it.
- **A09** findings are almost always Minor impact and therefore Low or Info on
  ...
  of a breach.
- **A05** findings about deployment state usually cannot be settled by code read
  alone. Under Rule 2 they land unverified at Info. That is the correct outcome,
  not a gap in the review. Record them in `unverified.md` with the question a
  human should answer.
